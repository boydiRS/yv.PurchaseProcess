...
@Component({
  selector: 'app-er-master-list',
  templateUrl: './er-master-list.component.html',
  styleUrls: ['./er-master-list.component.css']
})
export class ErMasterListComponent implements OnInit {
  type: string;
  id: string;
  ...
  loadData() {
    ...
    this.orderService.ERMaster_GetAll(this.searchData).subscribe(x => {
  ...
  loadDataForm() {
    this.dataFormService.DataForm_ERMasterList(this.userInfo.code).subscribe(x => {
      this.dataForm = x;
