...
@Component({
  ...
  styleUrls: ['./invoice-item-list.component.css']
})
export class InvoiceItemListComponent implements OnInit {
  ...
  loadData() {
    ...
    this.orderService.InvoiceItem_GetAll(this.searchData).subscribe(x => {
  ...
  loadDataForm() {
    this.dataFormService.DataForm_InvoiceItemList().subscribe(x => {
      this.dataForm = x;
    });
  }
