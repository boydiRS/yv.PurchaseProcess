...
@Component({
  selector: 'app-eo-master-list',
  templateUrl: './eo-master-list.component.html',
  styleUrls: ['./eo-master-list.component.css']
})
export class EoMasterListComponent implements OnInit {
  type: string;
  id: string;
  ...
  loadData() {
    if(this.ExportFromDate){ this.searchData.ExportFromDate = moment(this.ExportFromDate).format("MM/DD/YYYY");}
    else{this.searchData.ExportFromDate = '';}

    if(this.ExportToDate){this.searchData.ExportToDate = moment(this.ExportToDate).format("MM/DD/YYYY");}
    else{this.searchData.ExportToDate = '';}

    this.loadingIndicator = true;
    this.orderService.EOMaster_GetAll(this.searchData).subscribe(x => {
