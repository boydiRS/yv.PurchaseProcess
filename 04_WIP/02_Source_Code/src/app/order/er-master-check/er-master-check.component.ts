...
@Component({
  selector: 'app-er-master-check',
  templateUrl: './er-master-check.component.html',
  styleUrls: ['./er-master-check.component.css']
})
export class ErMasterCheckComponent implements OnInit {
  id: string;
  ...
  onLoadData() {
    ...
    this.orderService.ERMasterCheck_GetAll(this.searchData).subscribe(x => {
  ...
  loadDataForm() {
    this.dataFormService.DataForm_ERMasterCheckList().subscribe(x => {
      this.dataForm = x;
    });
  }
