...
export class UnitListComponent implements OnInit {
  ...
  loadPermission(){
    this.systemService.CheckPermission(WorkingProcessConst.APP_PPM_Unit, this.userInfo.code, null).subscribe(x => {
  ...
  loadPICPermission(){
    this.systemService.CheckPermission(WorkingProcessConst.APP_PPM_Unit, this.userInfo.code, 1).subscribe(x => {
  ...
  onLoadData() {
    ...
    this.systemService.Unit_GetAll(this.searchData).subscribe(x => {
