...
export class ProductGroupListComponent implements OnInit {
  ...
  loadData() {
    ...
    this.productService.ProductCategory_GetAll(this.searchData).subscribe(x => {
  ...
  loadDataForm() {
    this.dataFormService.DataForm_ProductCategoryList(this.userInfo.code).subscribe(x => {
      this.dataForm = x;
