...
@Component({
  selector: 'app-contract-pri-list',
  templateUrl: './contract-pri-list.component.html',
  styleUrls: ['./contract-pri-list.component.css']
})
export class ContractPriListComponent implements OnInit {
  id: string;
  ...
  onLoadData() {
    ...
    this.supplierService.ContractPrinciples_GetAll(this.searchData).subscribe(x => {
  ...
  onDelete(id){
    ...
    swalDelete.fire({
      ...
      if (result.value) {
        this.spinner.show();
        this.supplierService.ContractPrinciples_Delete(id).subscribe(x => {
          this.spinner.hide();
          if (x.ErrorCode !== '00') {
